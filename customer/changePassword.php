...
if(isset($_POST['save'])){
    ...
    if($conPassword != $newPassword){
        ...
        $query=mysqli_query($con,"select * from tblcustomers where Id='$customerId' and password='$currentPassword'");
        $row=mysqli_fetch_array($query);

        if($row > 0){

            $ret=mysqli_query($con,"update tblcustomers set password='$newPassword' where Id='$customerId'");
            if($ret == True){
